...
const parseBeatmap = (text) => {
    ...
    result._HitObjects = result.HitObjects.map(x => x);
    ...
    result._HitObjects_drawOrder = result.HitObjects_drawOrder.map(x => x);
...
const computeMapProperties = () => {
    ...
    beatmap.radius = 54.4 - 4.48 * clamp(0, parseFloat(beatmap.Difficulty.CircleSize) * (activeMods.has("ez") ? 0.5 : activeMods.has("hr") ? 1.3 : 1), 10);

    const ar = clamp(0, parseFloat(beatmap.Difficulty.ApproachRate) * (activeMods.has("ez") ? 0.5 : activeMods.has("hr") ? 1.4 : 1), 10);
    if (ar < 5) {
    ...
    beatmap.fadeout = 233; // fadeout is constant
}
...
const computeBreaks = (player) => {
    ...
    const lastObj = beatmap.HitObjects_drawOrder.at(-1);
    breaks = [];
...
export const toggleMod = (mod) => {
    ...
    switch (mod) {
        case "ez": {
            // incompatible mods
            if (activeMods.has("hr"))
            activeMods.delete("hr");
        ...
        case "hr": {
            // incompatible mods
            if (activeMods.has("ez"))
                activeMods.delete("ez");
        ...
        case "ht": {
            ...
            musicPlayer.changePlaybackRate(toggledOn ? 0.75 : 1);
        ...
        case "dt": {
            // incompatible mods
            if (activeMods.has("ht"))
                activeMods.delete("ht");

            musicPlayer.changePlaybackRate(toggledOn ? 1.5 : 1);
            if (!musicPlayer.paused)
                musicPlayer.play();
            break;
        }
